// FinanceHub - Financial Management App
class FinanceApp {
    constructor() {
        this.currentUser = {
            role: 'student',
    ...
    loadSampleData() {
        ...
        this.data.budgets = sampleData.budgetCategories;
    ...
    switchRole(role) {
        ...
        // Re-render dashboard with role-specific content
        this.renderDashboard();
    ...
    renderTrendChart() {
        ...
        // Create mock trend data for the last 7 days
        const days = [];
        const incomeData = [];
        const expenseData = [];
    ...
    handleTransactionSubmit(e) {
        ...
        // Update transactions if currently viewing them
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.addEventListener('keydown', (e) => {
        ...
        if (e.key === 'Escape') {
            document.querySelectorAll('.modal:not(.hidden)').forEach(modal => {
                window.financeApp.hideModal(modal.id);
            });
        }
        ...
        if ((e.ctrlKey || e.metaKey) && e.key === 'n') {
            e.preventDefault();
            window.financeApp.showModal('transactionModal');
        }
    });
    ...
    setTimeout(() => {
        if (window.financeApp) {
            window.financeApp.showNotification('Welcome to FinanceHub! 🎉 Start by exploring your dashboard or adding a transaction.', 'success');
        }
    }, 2000);
});